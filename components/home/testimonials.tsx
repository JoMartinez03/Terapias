...
export function Testimonials() {
  return (
    <section id="testimonios" className="bg-mist">
      <div className="wrap py-16 md:py-24 lg:py-28">
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <SectionLabel className="text-chocolate/70">Testimonios</SectionLabel>
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-3">
          {Array.from({ length: pending }).map((_, i) => (
            <Reveal key={i} delay={i * 70} className="h-full">
              <article className="flex h-full flex-col items-center gap-5 rounded-2xl border border-dashed border-sage/40 bg-almond p-7 text-center">
                <span
                  aria-hidden="true"
                  className="font-serif text-6xl leading-none text-chocolate/30"
                >
                  &ldquo;
                </span>
                <p className="flex-1 italic leading-relaxed text-chocolate/70">
                  Testimonio pendiente de confirmación con el equipo de Laura.
                </p>
                <p className="text-xs uppercase tracking-[0.25em] text-chocolate/50">
                  Próximamente
                </p>
              </article>
            </Reveal>
